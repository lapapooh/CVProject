...
namespace INTRANET.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginVM model, string returnUrl)
        {
            ...
            dynamic ADuser = ActiveDirectoryService.GetUser(model.UserID, model.Password);
            string role = GetDynamicProperty(ADuser, "Role");
            if (string.IsNullOrEmpty(role))
                role = "";

            if (ADuser != null && (role.Contains("staff") || role.Contains("student")))
            {
                ...
                var existingUser = await UserManager.FindByNameAsync(model.UserID);
                if (existingUser == null)
                {
                    var user = new ApplicationUser
                    {
                        UserName = model.UserID,
                        Email = GetDynamicProperty(ADuser, "Email"),
                        FirstName = GetDynamicProperty(ADuser, "FirstName"),
                        LastName = GetDynamicProperty(ADuser, "LastName")
                    };

                    var result2 = await UserManager.CreateAsync(user, model.Password);
                    if (result2.Succeeded)
                    {
                        var result3 = UserManager.AddToRole(user.Id, role);
                ...
                {
                    if (!UserManager.IsInRole<ApplicationUser, string>(existingUser.Id, role))
                    {
                        var result3 = UserManager.AddToRole(existingUser.Id, role);
                        if (result3.Succeeded)
                        {
